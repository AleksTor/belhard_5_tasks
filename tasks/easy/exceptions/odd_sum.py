...
def odd_sum(int_list: list) -> int:
    ...
    num = 0
    for i in int_list:
        if isinstance(i, int):  # ф-ция специально создана для проверки принадлежности к опред. кл.
            if i % 2 == 1:  # остаток должен быть нечетным, чтобы получить нечетные числа по условию
                num = num + i
        else:
